...
//      Should be faster for all small sets
//      Might have faster lookups than 'set' for large sizes
//      Slower inserts
...
public:
    ...
    {
        // Right now we're gonna be really lazy and just sort every time
        if (!count(val)) {
            data_.push_back(val);
    ...
    {
        // TODO: change this to a linear search if the set is super small
    ...
    {
        // TODO: change this to a linear search if the set is super small
        auto it = std::lower_bound(data_.cbegin(), data_.cend(), val);
        if (it == data_.cend())
            return it;
        else if (*it == val)
            return it;
